// scripts/test-s1-anchors.ts — S1 锚点效果测试（输出完整审计 JSON）
...
async function main() {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    const r = await runAIQualityAudit(1, output);
  ...
  const outFile = `temp/s1-anchor-test-${Date.now()}.md`;
