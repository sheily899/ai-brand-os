// scripts/test-s2-generic.ts — S2 通用版测试（无锚点）
...
async function main() {
  ...
  const rows = await db.select().from(stageRecord)
    .where(and(eq(stageRecord.projectId, "qbt_bOs495Sa5_74"), eq(stageRecord.stageNumber, 2)))
    .limit(1);
  const output = rows[0]?.structuredOutput as any;
  if (!output) { console.log("S2: 无数据"); process.exit(1); }

  const config = STAGE_AUDIT_CONFIGS[2];
  const hasAnchors = !!config.scoringAnchors;

  const out: string[] = [];
  out.push("# S2 通用版测试 — 完整审计结果\n");
  out.push(`项目: qbt_bOs495Sa5_74 | 阈值: ${config.advanceThreshold} | 锚点: ${hasAnchors ? "✅ 已启用" : "❌ 未启用"}\n`);
  out.push(`测试时间: ${new Date().toISOString()}\n`);

  out.push(`\n---\n`);
  out.push(`## S2 输入：商业背景分析 (Structured Output)\n`);
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    const r = await runAIQualityAudit(2, output);
    ...
    process.stdout.write(` S:${dims.specificity} D:${dims.differentiation} A:${dims.actionability} E:${dims.evidence}\n`);
  }
  ...
  const outFile = `temp/s2-generic-test-${Date.now()}.md`;
