// scripts/test-s6-generic.ts — S6 通用版测试（无锚点）
...
async function main() {
  ...
  const rows = await db.select().from(stageRecord)
    .where(and(eq(stageRecord.projectId, "qbt_bOs495Sa5_74"), eq(stageRecord.stageNumber, 6)))
    .limit(1);
  const output = rows[0]?.structuredOutput as any;
  if (!output) { console.log("S6: 无数据"); process.exit(1); }

  const config = STAGE_AUDIT_CONFIGS[6];
  const hasAnchors = !!config.scoringAnchors;

  const out: string[] = [];
  out.push("# S6 通用版测试 — 完整审计结果\n");
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    const r = await runAIQualityAudit(6, output);
  ...
  const outFile = `temp/s6-generic-test-${Date.now()}.md`;
