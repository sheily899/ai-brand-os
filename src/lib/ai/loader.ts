...
3. **影响层**：这些变化对你这个品牌具体意味着什么？`,
      ...
      BREAKTHROUGH_QUESTION: "如果把市场很大这几个字放一边——你亲眼看到的、让你确信该现在动手的那个信号是什么？",
...
3. **判断层**：这些观察指向什么机会方向？判断依据的可靠程度如何？`,
      ...
      BREAKTHROUGH_QUESTION: "如果市场机会这个词太抽象——你观察到的那个具体场景里，消费者现在最无奈的是什么？",
...
注意：Path B 降级路径的三轮限制（行为锚定→场景假设→反向排除）优先级高于三层递进。Path B 三轮结束后直接输出确认总结，不适用三层递进。`,
      ...
      BREAKTHROUGH_QUESTION: "你能想起一个具体的用户吗——Ta 在使用现有产品后的真实反应是什么样的？不用分析，就描述那个瞬间。",
...
3. **空位层**：现有竞争格局中，哪个需求维度尚未被充分覆盖？`,
      ...
      BREAKTHROUGH_QUESTION: "如果把更好、创新这些词放一边——在消费者的购买决策里，哪个环节现在没有任何品牌做好？",
...
当前模块三层问完后，切换到下一个模块。不允许在一个模块上无限追问。`,
      ...
      BREAKTHROUGH_QUESTION: '如果不考虑品牌该怎么说——你心里最想让消费者用一句话向朋友推荐你的时候，说的那句话是什么？',
...
3. **边界层**：哪些视觉方向与品牌核心明确冲突？为什么？`,
      ...
      BREAKTHROUGH_QUESTION: "如果不说高级感——你能不能想起一个具体的画面、一张图、一个场景，让你觉得就是这个感觉？",
...
3. **执行层**：每个渠道，内容的具体形式和表达重点是什么？`,
      ...
      BREAKTHROUGH_QUESTION: "如果一个用户关注了你一年，你觉得她会跟朋友说这个品牌的内容___——她会填什么词？",
