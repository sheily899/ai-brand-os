...
const STAGE_AUDIT_CONFIGS: Record<number, StageAuditConfig> = {
  ...
  2: {
    ...
    reoptimizeThreshold: 50,
    focusAreas: [
      "商业模式是否说清了产品形态、收入来源、交付方式和当前阶段（概念阶段/产品验证/增长期/规模化）",
  ...
  4: {
    ...
    focusAreas: [
      ...
      "功能层需求与身份认同层需求是否清晰区分且不重复",
      "现有解决方案的失效原因是否具体到用户行为（「用户试过X但放弃因为Y」而非「用户不满意」）",
    ...
    scoringAnchors: {
      ...
      differentiation: {
        score1: '需求为品类通用（好用/品质好），身份认同层需求=彰显品味/体现个性，无法体现目标用户的独特心理',
        score3: '明确区分功能层/身份认同层 需求，身份认同层需求含 ≥1 个非显性洞察（用户自己未意识到的深层动机）',
        score5: '身份认同层需求 ≥2 个洞察有行为/语言支撑，解释目标用户与普通消费者的关键差异，发现品类默认之外的隐藏需求',
      },
      actionability: {
      ...
      evidence: {
        ...
        score3: '来源：≥2 个洞察有来源支持（原话/反馈/观察/记录），区分已验证事实、创始人推断、待验证假设。可信度：≥1 个来源有行为记录支撑（非纯主观），失效原因有行为解释。推理链：≥1 条完整推理链「观察行为→解释行为→深层需求→品牌启示」',
  ...
  5: {
    ...
    focusAreas: [
      ...
      "竞品分析是否深入到具体短板和机会缺口——每个弱点能否追溯到用户评价原文或产品功能缺失",
    ...
    scoringAnchors: {
      specificity: {
        score1: '竞品名单无选择逻辑，分析停留在表面描述（「A品牌做得好」「B品牌价格低」），未深入到短板/缺口',
        score3: '竞品有选择理由且覆盖 ≥2 个竞争方向。≥2 个竞品含具体短板（可追溯到评价或功能缺失）。有竞争维度定义',
        score5: '覆盖 ≥3 个竞争方向各有代表性品牌。每个短板有 ≥1 条评价原文/功能对比支撑。市场机会可追溯到具体缺口',
      },
      differentiation: {
      ...
      actionability: {
        ...
        score3: '≥2 个机会缺口对应品牌行动方向（「竞品X不足→我们建Y→做法Z」），可转化为 S6 选择理由素材',
  ...
  6: {
    ...
    focusAreas: [
      ...
      "三层价值主张是否各自独立且有递进——功能层≠情绪层≠社会层，不存在语义重复",
      "品牌人格特质是否包含可测试的行为约束（应做/禁做 能评审设计稿/文案，排除具体设计选择）",
    ...
    scoringAnchors: {
      specificity: {
        ...
        score3: '四要素齐全（目标消费者含行为特征/品类有边界/价值非通用/理由有逻辑链），VP三层独立不重复，≥5 个特质各有 应做/禁做',
        score5: '可作为创意简报独立使用。目标消费者含 ≥2 个行为特征+场景。品牌故事有叙事逻辑（困境时刻+品牌行动+品牌关系）。≥3 个特质 应做/禁做 可转化为评审清单',
      },
      differentiation: {
        score1: '定位换名可用于任意同类品牌。VP均为品类通用诉求。品牌人格为品类刻板印象。brandStory为通用模板',
        score3: '≥1 个差异化要素（独特人群/非默认属性/独有资源）。≥2 个特质可区分竞品。≥1 条 价值主张 触及未被表达的消费者需求',
        score5: '完整差异化系统——人群对应 S5 空位、价值形成认知差异、≥3 个特质成自洽体系、≥2 条 价值主张 可追溯到 S4/S5 具体 gap',
      },
      actionability: {
        score1: '无法指导 S7/S8——定位太模糊、应做/禁做 不可测试（「做好产品」/「做不好产品」）、VP 无法生成内容主题',
        score3: '可写入创意简报：目标消费者定义能指导 S7 方向，≥3 个特质 应做/禁做 可测试，≥2 条 价值主张 可转化为内容主题，品牌故事有 ≥1 个可传播元素',
        score5: '即完整创意简报：定位可生成一句话 创意简报，应做/禁做 覆盖 ≥3 个决策领域，每条 价值主张 可展开为内容支柱，推理依据 链为 S7/S8 提供可追溯依据',
      },
      evidence: {
        score1: '来源：推理依据 三个引用全部标注「未追溯」或仅有模糊引用（「来自S4」但无字段/内容）。推导链断裂——S6 与 S3/S4/S5 无逻辑关系。可信度：前序引用无法验证。推理链：无。品牌定位与消费者需求/竞争空位之间无逻辑链',
        score3: '来源：≥2 个 推理依据 引用标注了具体字段名+内容摘要。可信度：前序字段确实存在且内容匹配引用描述。推理链：≥1 条完整推导链「S4发现X→S5确认竞品未解决X→S6定位Y」。品牌故事与 S1/S4 记录的消费者困境一致',
        score5: '来源：全部 3 个引用有字段名+内容摘要+推导逻辑。品牌故事要素可追溯到 S1/S4。每条 价值主张 有独立 S4/S5 来源。可信度：每个引用的前序字段可交叉验证，引用内容与原文一致。推理链：完整推导链：S4消费者需求→S5竞争空位→S6品牌定位。消费者描述与 S4 消费者画像一致，核心价值方向与 S5 市场机会一致',
      },
    },
  },
  ...
  7: {
    ...
    reoptimizeThreshold: 60,
    focusAreas: [
      "核心视觉概念 是否统领性地定义了视觉美学方向，而非仅列出视觉元素",
      "五种视觉语言（形态/色彩/字体/图像/材质）是否全部填充且每种有 ≥2 个具体样式描述",
      "视觉方向是否与 S6 品牌人格一致——品牌人格的应做/禁做是否在视觉语言中有对应体现",
    ...
    scoringAnchors: {
      specificity: {
        ...
        score3: '五种全填充，≥3 种有具体描述（色值范围/字体风格/摄影方向），核心视觉概念 有明确美学方向',
        score5: '五种各 ≥2 个样式描述或参考，视觉系统内部一致（互相呼应），核心视觉概念 统领全局，有应用场景说明',
      },
      differentiation: {
      ...
      actionability: {
        ...
        score5: '五种全有可交付描述+参考方向+情绪板，restrictions 可作为完整设计评审 检查清单，有视觉优先级指导',
      },
      evidence: {
        score1: '来源：无战略依据——所有视觉选择均为 AI 审美判断（「暖色=温暖」），与 S6 无关联。无法回答「为什么选这个方向」。可信度：无。推理链：视觉方向与品牌战略之间无推理链',
        score3: '来源：≥2 个关键视觉决策可追溯到 S6 或 S4 具体字段。≥1 个 restrictions 有战略理由。可信度：引用的前序字段确实存在且与视觉选择逻辑一致。推理链：≥1 条推理链「品牌人格/定位→视觉决策」。核心视觉概念 推导可追溯到品牌战略',
        score5: '来源：所有关键决策（核心视觉概念+五种语言）有明确的战略依据。每个视觉选择可追溯到 S6/S4/品牌人格的具体字段。可信度：战略依据真实存在，视觉选择的推导过程可独立验证。推理链：完整「从战略到视觉」推导链。消费者心理→品牌人格→视觉表达每步有明确逻辑',
      },
    },
  },
  ...
  8: {
    ...
    focusAreas: [
      ...
      "内容价值体系四阶段（认知/兴趣/信任/决策）是否覆盖完整且各有具体内容策略",
...
证据来源检查：推理依据 引用是否显式标注了 S3/S4/S5 的具体字段名和内容摘要？
...
- **评分理由中严禁使用任何英文词汇或缩写**，包括但不限于：Presence、Reliability、Connection、Fact、Inference、Hypothesis、verified、inferred、explicit、evidenceLevel、founder_observation、VP（请写"价值主张"）、应做/禁做（请写"行为准则/禁区"）、核心视觉概念（请写"核心概念"）、推理依据（请写"推理依据"）、weakness（请写"弱点"）、gap（请写"缺口"）、KPI（请写"关键指标"）、检查清单（请写"检查清单"）、情绪板（请写"情绪板"）、functional/identity needs（请写"功能需求/身份需求"）、consumerProfile（请写"消费者画像"）、marketOpportunity（请写"市场机会"）、restrictions（请写"视觉禁区"）、contentValueSystem（请写"内容价值体系"）、opportunityGap（请写"机会缺口"）、failReason（请写"失败原因"）、fulltext、snippet、user review。评分理由必须是纯中文自然语言。
